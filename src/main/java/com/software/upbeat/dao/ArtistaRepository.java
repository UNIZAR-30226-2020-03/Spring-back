...
public interface ArtistaRepository extends JpaRepository<Artista, Long>{
	@Query(value = "SELECT * FROM artista a WHERE a.correo = ?1", nativeQuery = true)
	Artista findByEmail(String correo);
	
	@Query(value = "SELECT * FROM artista a WHERE a.contrasenya = ?1 AND a.correo = ?2", nativeQuery = true)
